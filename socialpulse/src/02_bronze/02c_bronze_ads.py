...
if is_full_refresh:
    df = spark.read.table('system.information_schema.tables').filter(fn.col('table_name') == 'raw_ads').select(fn.concat(fn.col('table_catalog'),fn.lit('.'),fn.col('table_schema'),fn.lit('.`'),fn.col('table_name')).alias('full_qualified_name'))
...
@dp.table(
    name    = 'socialpulse_catalog.bronze.raw_ads',
